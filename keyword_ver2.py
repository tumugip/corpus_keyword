...
def keyword(corpus_df):
    ...
    for row in corpus_df.itertuples():
        JP_Sentence = row[1].replace('option: ','')
        JP_Sentence_Mecab = MeCab.Tagger().parse(JP_Sentence)
        # print(JP_Sentence_Mecab)
        JP_Sentence_Mecab = JP_Sentence_Mecab.split('\n')
        count = 0
        for wakati in JP_Sentence_Mecab:
            wakati2 = wakati.split('\t')
            JP_Sentence_Mecab[count] = wakati2
            count+=1
        # print(JP_Sentence_Mecab)
        sentence_noEOS = []
        for i in range(len(JP_Sentence_Mecab)):
            if len(JP_Sentence_Mecab[i]) == 8:
                sentence_noEOS.append(JP_Sentence_Mecab[i])
        # print(sentence_noEOS)
        JP_keyword = []
        for i in range(len(sentence_noEOS)):
            JP_hinsi = sentence_noEOS[i][4].split('-')
            if JP_hinsi[0] != '助詞':
                JP_keyword.append(sentence_noEOS[i][0])
        JP_keyword = ' '.join(JP_keyword)
        # print(JP_keyword)
        corpus_list = [JP_keyword,row[2]]
        corpus = pd.DataFrame(corpus_list)
        keyword_corpus = pd.concat([keyword_corpus, corpus.T])
    keyword_corpus = keyword_corpus.reset_index(drop=True)
